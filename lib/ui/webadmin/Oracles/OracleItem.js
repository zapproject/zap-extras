...
export class OracleItem extends React.Component {
  ...
  render() {
    ...
    const link = `#${withBroker ? ViewsEnum.TOKEN_PROVIDERS_LIST : ViewsEnum.ORACLES};search=${hash.search};page=${hash.page};bonded=${hash.bonded ? 'true' : ''};oracle=` + (expanded ? '' : oracle.address); // const link = `#' + ViewsEnum.ORACLES + (expanded ? '' : ';' + oracle.address);
