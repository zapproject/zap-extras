...
const restrictedAddresses = ['0xaD0Adf0C81E9c18D5DE0D6D5555A909c6435062D', '0x2416002D127175BC2d627FAefdaA4186c7c49833', '0x47834a7533Eb6CB6F8ca1677405423e476cE3f31', '0x6Be845635029C8C8F44bc8d729624d0c41adCcDE', '0xf108237201E6EE3906f19390C699dA4d1495040F'];
export class ProvidersService {
  constructor(networkId, networkProvider, web3) {
    ...
    _defineProperty(this, "bondage", void 0);

    _defineProperty(this, "endpointsPromise", void 0);

    _defineProperty(this, "allOraclesPromise", void 0);

    _defineProperty(this, "allProviderAddressesPromise", void 0);

    _defineProperty(this, "allEndpointsWithBrokerPromise", void 0);

    this.registry = new ZapRegistry(this.networkOptions);
    this.bondage = new ZapBondage(this.networkOptions);
    this.updateProvidersData();
    this.registry.listenNewProvider({}, () => {
      this.updateProvidersData();
    });
    this.registry.listenNewCurve({}, () => {
      this.updateProvidersData();
    });
  }
  ...
  async getProvidersWithTitles(withBroker = false, emptyProviders = false) {
    const providers = await this.allProvidersWithTitles;
    if (!withBroker) return providers;
    const [providerAddressesFromBrokerEndpoints, providerAddressesFromEndpoints] = await Promise.all([this.allEndpointWithBroker.then(endpoints => endpoints.map(e => e.provider)), this.allEndpoints.then(endpoints => endpoints.filter(e => e.endpoints.length > 0).map(e => e.provider))]);
  ...
  async getProviderEndpoints(provider, withBroker = false) {
  ...
  updateProvidersData() {
    ...
    this.allProviderAddressesPromise = Promise.all([this.registry.getAllProviders().then(addresses => addresses.filter(address => restrictedAddresses.indexOf(address) === -1)), sortedProvidersPromise]).then(([allProviders, providersSortedByDots]) => {
  ...
  async updateEndpoints(providers, sortedProvidersWithEndpoints) {
    const endpoints = await Promise.all(providers.map(provider => provider.getEndpoints()));
  ...
  loadTokenDotFactoryAndProvider(address) {
  ...
  getBoundDots(account, provider, endpoints) {
  ...
  async getProvidersBonded(account, search) {
    const providersWithEndpoint = await this.bondage.contract.getPastEvents('Bound', {
      filter: {
        holder: account
      },
      fromBlock: 0,
    ...
    const providers = providersWithEndpoint.map(provider => this.loadProvider(provider.address)); // load titles and bound dots in parallel

    const [_, allDots] = await Promise.all([this.providersWithTitles(providers), Promise.all(providersWithEndpoint.map(provider => this.getBoundDots(account, provider.address, provider.endpoints.map(e => e.endpoint))))]);
    ...
    providers.forEach((provider, providerIndex) => {
      const allEndpointsWithDots = providersWithEndpoint[providerIndex].endpoints.map(e => e.endpoint).map((endpoint, endpointIndex) => ({
        endpoint,
        dots: allDots[providerIndex][endpointIndex]
      })).filter(filterEndpointsWithDotsByDots).sort(sortEndpointsWithDots);
      const searchEndpointsWithDots = search ? allEndpointsWithDots.filter(filterEndpointsWithDotsBySearch) : []; // if no endpoint match the search, but provider matches, show all it's endpoints

      const endpointsWithDots = searchEndpointsWithDots.length ? searchEndpointsWithDots : allEndpointsWithDots;
      if (!!search && !searchEndpointsWithDots.length && provider.title.toLocaleLowerCase().indexOf(search) === -1 && provider.providerOwner.toLocaleLowerCase().indexOf(search) === -1) return;
      const dots = [];
      const endpoints = [];
  ...
  async getProviders(start = 0, withBroker, limit, skipProvidersWithoutEndpoints = false) {
    const [allProviders, allEndpoints, brokerEndpoints] = await Promise.all([this.allProviders, this.allEndpoints, this.allEndpointWithBroker]);
    const providersWithEndpoints = allProviders.map(provider => ({
      provider,
      endpoints: ProvidersService.getEndpointsByOracleAddress(allEndpoints, provider.providerOwner)
    }));
  ...
  async search(text, start = 0, withBroker, limit, skipProvidersWithoutEndpoints = false) {
    if (!text.length) this.getProviders(start, withBroker, limit);
    await this.allProviders;
    const [allEndpoints, providers] = await Promise.all([this.allEndpoints, this.allProvidersWithTitles // for search we need titles of all providers
    ]);
    ...
    providers.forEach(provider => {
      ...
      const searchEndpoints = allProviderEndpoints.filter(endpoint => endpoint.toLowerCase().indexOf(text) !== -1); // if no endpoint match the search, but provider matches, show all it's endpoints

      const endpoints = searchEndpoints.length ? searchEndpoints : allProviderEndpoints;
      if (!searchEndpoints.length && provider.title.toLocaleLowerCase().indexOf(text) === -1 && provider.providerOwner.toLocaleLowerCase().indexOf(text) === -1) return;
